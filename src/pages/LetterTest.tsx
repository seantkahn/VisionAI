...
const LetterTest: React.FC = () => {
  ...
  const { testMode, eyeToExamine, distance } = location.state || {};
  ...
  const diopters = [0.0, 0.5, 1.0, 1.5, 2.0, 2.75, 4.0, 6.0];
  ...
  const updateRandomIcons = () => {
    ...
    if (newCount > 7) {
      ...
      const currentGreenLetterCount = randomString.filter(
        (obj) => obj.recognized
      ).length;
  ...
  const handleLetterButtonClick = (letter: string) => {
    ...
    keyboardButtons.forEach((button) => {
      if (button.textContent === letter) {
        ...
        const isRecognized = updatedRandomString.find(
      ...
      <IonContent className="ion-padding">
        ...
        <IonText style={{ textAlign: "center" }}>
          <h1 style={{ fontWeight: "bold" }}>
            Letter Test: {buttonPressCount}/7
          </h1>
        </IonText>
        <div className="letter-test-camera"></div>
          ...
          <div className="keyboard-row">
            {Array.from("QWERTYUIOP").map((letter, index) => (
              <button
                ...
                style={{
                  backgroundColor: randomString.find(
                    (obj) => obj.letter === letter
                  )?.recognized
                    ? "green"
                    : "",
